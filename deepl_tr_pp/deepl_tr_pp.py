r"""Translate text via deepl up to 5000 chars.
...
async def deepl_tr_pp(  # noqa: C901
    ...
    # slash / does not play well, replace with " "
    try:
    ...
    # ".lmt__translations_as_text__text_btn"  htlm()
    ...
    sep_ = sep.strip()
    if not sep_:  # handle for sep="\n"
        sep_ = "\n"

    logger.debug("res: %s", res.split(sep_)[-3:])
    ...
    if not debug:
        ...
        deepl_tr_pp.text = text
        deepl_tr_pp.url_ = url_
        deepl_tr_pp.content = content
        deepl_tr_pp.res = res
